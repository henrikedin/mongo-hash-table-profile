...
template <typename K_L, typename K_S, typename V, typename Traits>
inline auto UnorderedFastKeyTable<K_L, K_S, V, Traits>::insert(const HashedKey& key) -> std::pair<iterator, bool> {
...
inline auto UnorderedFastKeyTable<K_L, K_S, V, Traits>::try_emplace(const HashedKey& key,
	...
	throw std::runtime_error("UnorderedFastKeyTable couldn't add entry after growing many times");
}
