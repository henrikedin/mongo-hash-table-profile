...
public:
	...
	float load_factor() const {
		return static_cast<float>((double)size() / capacity());
	}
	...
	std::pair<iterator, bool> insert(const HashedKey& key);
	std::pair<iterator, bool> insert(const K_L& key) {
		return insert(HashedKey(key));
	}

	std::pair<iterator, bool> emplace(const HashedKey& key)
	{
		return insert(key);
	}
	std::pair<iterator, bool> emplace(const K_L& key) {
		return insert(HashedKey(key));
	}
